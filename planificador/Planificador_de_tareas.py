...
def exportar_csv(nombre_archivo):
    ...
    with open(nombre_archivo, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["id", "titulo", "descripcion", "fecha_limite", "prioridad"])
        for tarea in tareas:
...
def importar_csv(nombre_archivo):
    with open(nombre_archivo, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            agregar_tarea(row.get('titulo', ''), row.get('descripcion', ''), row.get('fecha_limite', ''), row.get('prioridad', ''))

def exportar_json(nombre_archivo):
    tareas = obtener_tareas()
    data = [{"id": t[0], "titulo": t[1], "descripcion": t[2], "fecha_limite": t[3], "prioridad": t[4]} for t in tareas]
...
def importar_json(nombre_archivo):
    with open(nombre_archivo, 'r', encoding='utf-8') as f:
        data = json.load(f)
        for item in data:
            agregar_tarea(item.get('titulo',''), item.get('descripcion',''), item.get('fecha_limite',''), item.get('prioridad',''))
# ------------------- UTILIDADES -------------------
